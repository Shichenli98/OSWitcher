...
def dynamic_fit_conv(kernel_size, input_width, input_height, input_channel=3, output_channel=2):
    print('fit ', kernel_size,input_width, input_height)
    ...
    print('torch ', torch_time, 'fft ', fft_time)
...
def dynamic_convert_opt_conv2d(module, in_shape):
    org_in_batch,org_in_channels, org_in_width, org_in_height = in_shape
    # summarize output sizes per layer
    mySummary = summary(module, in_shape).summary_list
...
if __name__ == "__main__":
    ...
    opt_myNet = dynamic_convert_opt_conv2d(myNet, image.shape)
